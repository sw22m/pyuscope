...
def except_wrap(func):
    ...
    def wrapper(**kwargs):
        try:
            current_app.plugin.log_verbose(f"{request.url}")
...
def make_app(app):
    ...
    @except_wrap
    def position():
        position = current_app.plugin.position()
        return {
            'data': position,
    ...
    def move_absolute():
        ...
        for axis in "xyz":
            ...
            if this_pos is not None:
                data[axis] = this_pos
        current_app.plugin.move_absolute(data, block=block)
    ...
    def move_relative():
        ...
        for axis in "xyz":
            ...
            if this_pos is not None:
                data[axis] = this_pos
        current_app.plugin.move_relative(data, block=block)
        return {'status': HTTPStatus.OK}
    ...
    @except_wrap
    def is_idle():
        is_idle = current_app.plugin.is_idle()
        return {
            'data': is_idle,
            'status': HTTPStatus.OK,
        }
    ...
    def pyuscope_version():
        pyuscope_version = current_app.plugin.pyuscope_version()
        return {
    ...
    @except_wrap
    def objectives():
        objectives = current_app.plugin.get_objectives_config()
        return {
            'data': objectives,
            'status': HTTPStatus.OK,
        }
    ...
    def active_objective_set(objective):
        ...
        if objective not in current_app.plugin.objectives.names():
            current_app.plugin.log(f"WARNING: objective '{objective}' not found")
            return {'status': HTTPStatus.BAD_REQUEST}
        current_app.plugin.set_active_objective(objective)
        return {'status': HTTPStatus.OK}

    @app.route('/get/imager/imager/get_disp_properties', methods=['GET'])
    @except_wrap
    def imager_get_disp_properties():
        properties = current_app.plugin.imager_get_disp_properties()
        return {
            'data': properties,
            'status': HTTPStatus.OK,
        }
    ...
    def imager_get_disp_property(name):
        val = current_app.plugin.imager_get_disp_property(name)
        return {
            'data': val,
            'status': HTTPStatus.OK,
        }
    ...
    def imager_set_disp_properties():
        ...
        # Validate objective name before sending request
        current_app.plugin.imager_set_disp_properties(properties)
        return {'status': HTTPStatus.OK}
    ...
    def autofocus():
        block = bool(request.args.get("block", default=True, type=int))
        current_app.plugin.autofocus(block=block)
        return {'status': HTTPStatus.OK}
    ...
    def subsystem_functions():
        return {
            'data': current_app.plugin.subsystem_functions_serialized(),
            'status': HTTPStatus.OK,
        }
    ...
    def subsystem_function(subsystem, function):
        ...
        current_app.plugin.subsystem_function_serialized(subsystem_=subsystem,
                                             function_=function,
                                             **kwargs)
        return {'status': HTTPStatus.OK}
    ...
    @except_wrap
    def wait_imaging_ok():
        current_app.plugin.wait_imaging_ok()
        return {
            'status': HTTPStatus.OK,
        }
